...
/*
 * @class ClazzCourseSearchProps
 * @description 学生课程管理界面的搜索组件的props
 * @author 戴俊明 <idaijunming@163.com>
 * @date 2019/8/10 20:11
 **/
interface ClazzCourseSearchProps extends FormComponentProps {
	/*
	 * @var 学生id
	 * @author 戴俊明 <idaijunming@163.com>
	 ...
	 **/
	onSearch: (id: number, search: Partial<ClazzCourseModel>) => void
	/*
	 * @var 点击批量选择按钮后触发的回调
	 * @author 戴俊明 <idaijunming@163.com>
	 * @date 2019/8/10 20:13
...
/*
 * @class ClazzCourseSearchContent
 * @description 学生课程管理界面的搜索组件,该组件需要Form包裹
 * @author 戴俊明 <idaijunming@163.com>
 * @date 2019/8/10 20:27
 **/
const ClazzCourseSearchContent: React.FC<ClazzCourseSearchProps> = (props) => {
	...
	useEffect(() => {
		...
		ClazzService.selectByName('').then(res => {
			const data = (res.data.data as Clazz[]).map(clazz => ({
				text: `${clazz.id} - ${clazz.name}`,
	...
	const handleSearch = debounce((value: string) => {
		...
		ClazzService.selectByName(value).then(res => {
			const data = (res.data.data as Clazz[]).map(clazz => ({
				text: `${clazz.id} - ${clazz.name}`,
				value: String(clazz.id)
			}));
...
const ClazzCourseSearch = Form.create<ClazzCourseSearchProps>({})(ClazzCourseSearchContent);

export default ClazzCourseSearch;